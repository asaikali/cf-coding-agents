...
| [`04-claude-ts-agent/`](04-claude-ts-agent/) | Scenario 3's same workflow re-implemented against the **TypeScript** Claude Agent SDK with `nodejs_buildpack` | Isolates the SDK-initialize hang documented in [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md) to the Python SDK's native-subprocess code path. Same agent prompt, same options, same UPS secrets, same target repo — only the language and release buildpack differ. This one **works on CF** where scenarios 2 and 3 don't. |
