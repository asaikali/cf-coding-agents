"""End-to-end smoke test for the droplet.

Drives the SDK through a strict verification sequence: clone the target repo,
compile it, boot the app, curl it, shut it down. Proves every link in the
chain — git HTTPS auth, JDK, Maven wrapper, Spring Boot startup, local HTTP
— plus the SDK wiring itself. Pure verification; no file edits, no fixes.
"""
...
# Point the SDK at its bundled claude binary; the default search path doesn't
# cover the droplet's layout. See agent.py for the longer note.
...
async def main() -> None:
    ...
    async for message in query(prompt=build_prompt(repo), options=options):
