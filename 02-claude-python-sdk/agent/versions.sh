#!/usr/bin/env bash
# Runs inside a cf task. Prints versions of every tool and dependency the
# agent relies on, plus a credentialed git clone probe. Missing tools are
# reported as NOT FOUND rather than halting, so one gap doesn't hide the
# rest.
...
# Claude Agent SDK is installed by python_buildpack from pyproject.toml.
# Use importlib.metadata so we don't depend on the package exposing __version__.
echo "== claude-agent-sdk =="
uv run python -c 'from importlib.metadata import version; print(version("claude-agent-sdk"))' 2>&1 \
  || echo "NOT FOUND"
echo

# Env delivered by .profile.d/vcap.sh out of VCAP_SERVICES.
echo "== env =="
...
# Same credentialed git clone probe as scenario 1: proves VCAP_SERVICES is
# parsed, GH_TOKEN is exported, and gh auth setup-git wired git's credential
# helper. Prefers a private repo so the auth path is actually exercised.
echo "== git clone (auth) =="
tmp_repo="$(mktemp -d)"
